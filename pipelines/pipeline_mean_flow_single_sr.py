...
from models.transformer_sd3_interval import SD3Transformer2DModel_interval, LogitNormalCosineScheduler
...
class StableDiffusion3ControlNetPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __init__(
        self,
        transformer: SD3Transformer2DModel_interval,
        scheduler: FlowMatchEulerDiscreteScheduler,
        vae: AutoencoderKL,
        text_encoder: CLIPTextModelWithProjection,
        tokenizer: CLIPTokenizer,
        ...
        self.get_time_scheduler = LogitNormalCosineScheduler(
                loc=0.0,
                scale=1.0,
                min_t=1e-4,
                max_t=1-1e-4
    ...
    def __call__(
        ...
        # 4. Prepare timesteps
        timesteps = self.get_time_scheduler.create_cosine_schedule(num_inference_steps, device)
        timesteps = timesteps.flip(0)  # 1 → 0
        ...
        if start_point == 'noise':
            ...
            sigmas = torch.full_like(sigmas, 0.5)
            sigma = sigmas[0].flatten()
            while len(sigma.shape) < 4:
                sigma = sigma.unsqueeze(-1)
    
            latents = latents_condition_image

        # 8. Denoising loop
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            # print(f"-------latent_size:{latents.size()}")
            ...
            for i, t in enumerate(timesteps):
                ...
                timestep_r = t1.expand(latent_model_input.shape[0])
                timestep_t = t0.expand(latent_model_input.shape[0])
                # r = timesteps[1].expand(latent_model_input.shape[0])
                # t = timesteps[0].expand(latent_model_input.shape[0])
